...
- form 태그는 action 설정을 하지 않아도 자체적으로 가지고 있는 주소가 있음
- 동기통신으로 서버에서 다른 페이지를 요청하지 않는한 다른페이지 이동불가
- `location.href` 태그와 같이 다른 페이지로 이동하는 코드는 실행되지 않음
- 비동기 통신에서는 상기 사유로 form 대신 div를 사용함
